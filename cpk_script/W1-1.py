...
#file_patterns = (("HUB4","*.W11*"),)
#file_patterns = (("HUB4","*.check*"),)
file_patterns = (("HUB4","*.tpt*"),)
...
for i in sk:
    for model_name, file_pattern in file_patterns:
        ...
        deviceList = CreateDeviceList(WalkDir(path,file_pattern)); #raw_input(deviceList)
        current_sn="xxxxx"
        ...
        for basename in deviceSorted:
            ...
            if fn:
                fn_data = open(fn).readlines(); raw_input(fn_data)
            else:
                fn_data = [""]
            '''
            fn_data = open(deviceList[basename]).readlines()
