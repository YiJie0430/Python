...
def Check_boot_testmode(parent,dut_id,term_arm,term_atom,log):
    parent.SendMessage(dut_id,"Waitting for DUT bootup...\n",log)
    test_time_=time.time()
    flag=0
    while 1:
       data_atom=term_atom.read(term_atom.inWaiting()); print data_atom
       data_arm=term_arm.read(term_arm.inWaiting()); print data_arm       
       if not len(data_atom) and not len(data_arm): flag+=1; time.sleep(3); print flag
       else: flag=0
       if time.time()-test_time_ > 50 or flag>2: 
          lWaitCmdTerm(term_arm,"qu","#",5)
          lWaitCmdTerm(term_atom,"root","#",5)
          data=lWaitCmdTerm(term_arm,"ifconfig","#",5)
          if 'lo' not in data: raise Except("FAIL: DUT Boot Fail")
          else: break
    parent.SendMessage(dut_id,"DUT bootup success...\n",log)       

def Enter_Testmode(parent,dut_id,term,log):
    data = lWaitCmdTerm(term,"md5sum /nvram/1/DownstreamCal/Odin","#",5,2); print data
    md5 = data.split("\r\n")[1].split("/nvram")[0].strip()
...
def Get_md5(parent,dut_id,term,log):
    data = lWaitCmdTerm(term,"md5sum /nvram/1/DownstreamCal/Odin","#",5,2); print data
    md5 = data.split("\r\n")[1].split("/nvram")[0].strip()
    #lWaitCmdTerm(term,'testmode',"#",20)
    #lWaitCmdTerm(term,'cli',">",10)
...
def mxl277_cmoa_csn_loop(parent,dut_id,term,basePower,bandwidth,log):
    '''
    author: YiJie Wang
    date: 2016/12/15
    ...
    result=0
    test_time = time.time()
    md5_list = [0 for j in xrange(100)] 
    for cnt in range(100):
        ...
        lWaitCmdTerm(term,"top",">",5,2)
        lWaitCmdTerm(term,"exit","#",5,2)
        data = lWaitCmdTerm(term,"md5sum /nvram/1/DownstreamCal/Odin","#",5,2)
        md5_list[cnt] = data.split("\r\n")[1].split("/nvram")[0].strip()
        if cnt > 0:
            msg = "Count %d Odin md5 = %s (<> %s Count %d Odin md5)"%(cnt,md5_list[cnt],md5_list[cnt-1],cnt-1) 
            if md5_list[cnt] == md5_list[cnt-1]: raise Except("FAIL: %s"%msg) 
            else: parent.SendMessage(dut_id,'%s\n'%msg,log,2)
        else:
            msg = "Count %d Odin md5 = %s "%(cnt,md5_list[cnt]) 
            parent.SendMessage(dut_id,'%s\n'%msg,log,2) 
        lWaitCmdTerm(term,"cli","mainMenu>",5,2)
        lWaitCmdTerm(term,"docsis","docsis>",5,2)
        lWaitCmdTerm(term,"Production","Production>",5,2)
        lWaitCmdTerm(term,"Calibration","Calibration>",5,2)
        '''
        parent.SendMessage(dut_id,'Count %d Downstream calibration Test Done\n'%cnt,log)
        parent.SendMessage(dut_id,"Count %d DS Calibration time: %3.2f (sec)\n"%(cnt,(time.time()- test_time)) ,log)
        parent.SendMessage(dut_id,"---------------------------------------------------------------------------\n",log)

def mxl277_cmoa_csn(parent,dut_id,term,basePower,bandwidth,cnt,log):
    '''
    author: YiJie Wang
    date: 2016/12/15
    description: *calibrate 5 diplexer for cmoa-csn
    ...
    #lWaitCmdTerm(term,"setstartup -e","#",5,2)
    #print 'comoa-tri' 
    if not result and cnt<100:
      data = lWaitCmdTerm(term,"md5sum /nvram/1/DownstreamCal/Odin","#",5,2); print data
      md5 = data.split("\r\n")[1].split("/nvram")[0].strip()          
      parent.SendMessage(dut_id,'Downstream calibration Test Done, Odin_md5: %s\n'%md5,log)
    elif 99<cnt<200: data=lWaitCmdTerm(term,"sync","#",5,2); parent.SendMessage(dut_id,'%s\nDownstream calibration Test Done\n'%data,log)
    else: time.sleep(3); parent.SendMessage(dut_id,'wait more 3 sec...\nDownstream calibration Test Done\n',log)
...
def dload(parent,dut_id,term_arm,term_atom,log):
    parent.SendMessage(dut_id,'Progress: fboot loading...\n',log,2)
    lWaitCmdTerm(term_arm,'quit','#',15,5)
    lWaitCmdTerm(term_arm,'./etc/scripts/sys_startup.sh','P7',40,5)
    lWaitCmdTerm(term_arm,'umount /nvram','#',5,5)
    lWaitCmdTerm(term_arm,'brctl addif br0 ngmii4','#',5,5)    
    #lWaitCmdTerm(term_atom,'root','#',5)
    lWaitCmdTerm(term_atom,'ifconfig gmac5 192.168.100.3','#',5,5)
    lWaitCmdTerm(term_atom,'cd /var/tmp','#',5,5)
    lWaitCmdTerm(term_atom,'mkdir NFS','#',5,5)
    lWaitCmdTerm(term_atom,'mount -t nfs 192.168.100.5:/home/johnny/NFS NFS','svc:',5,50)
    data=lWaitCmdTerm(term_atom,'cd NFS && sh -x ./atom_script','+ sync',80,5)
    #lWaitCmdTerm(term_atom,'dd if=NFS/CMOA-TRI-2.0.1.46.fboot of=/dev/mmcblk0','469504+0 records out',60,5)
    #term_arm.write('quit\n'); time.sleep(5)
    #term_arm.write('./etc/scripts/sys_startup.sh\n'); time.sleep(40)
    #term_arm.write('umount /nvram\n'); time.sleep(2)
    #term_arm.write('brctl addif br0 ngmii4\n'); time.sleep(2)
    #term_atom.write('root\n'); time.sleep(1)
    #term_atom.write('ifconfig gmac5 192.168.100.3\n'); time.sleep(1)
    #term_atom.write('cd /var/tmp && mkdir NFS\n'); time.sleep(1)
    #term_atom.write('mount -t nfs 192.168.100.5:/home/johnny/NFS NFS\n'); time.sleep(1)
    #term_atom.write('mount -t nfs 192.168.100.5:/home/johnny/NFS NFS\n'); time.sleep(1)
    #term_atom.write('cd NFS\n'); time.sleep(1)
    #term_atom.write('dd if=CMOA-TRI-2.0.1.46.fboot of=/dev/mmcblk0; sync\n'); time.sleep(60)    
    parent.SendMessage(dut_id,'%s\n'%data,log,2)
...
def T2(parent):
    try:
        ...
        start_time = end_time = 0
        #term =serial.Serial(comport[dut_id-1],b_rate)
        term_arm=serial.Serial('com1',b_rate)
        term_atom=serial.Serial('com2',b_rate)
        mac = GetMacAddress(parent,dut_id)
        #sn = GetSN(parent,dut_id)        
        log = log_counter(mac,FunctionName)
        start_time=time.time()
        ...
        for cnt in range(300):
            flag=1; parent.SendMessage(dut_id,"************** Round %d **************\n"%(cnt+1),log)            
            for loop in xrange(2):
                if flag: Check_boot_testmode(parent,dut_id,term_arm,term_atom,log); md5=Get_md5(parent,dut_id,term_arm,log)
                if loop:
                   msg = "Round %d Odin md5 = %s (<> default Odin md5: %s)"%((cnt+1),md5,default_table_md5) 
                   if default_table_md5 == md5 and flag: 
                      parent.SendMessage(dut_id,'Result: Round %d Odin was not overwriting (FAIL)\n\n'%(cnt+1),log,1); #raise Except("FAIL: %s"%msg) 
                      if cnt<100: failcount+=1
                      elif 99<cnt<200: failcount_+=1
                      else: failcount__+=1
                      Snmp.SnmpSet("192.168.1.10","1.3.6.1.4.1.26104.1.1.1.5.1.31","i",3,community="public")  #.24 #Power 1 , .25 Power 2, .31 power 8
                      time.sleep(3)
                   else:                       
                      if flag:
                         parent.SendMessage(dut_id,'Progress: %s\n'%msg,log,2)                      
                         dload(parent,dut_id,term_arm,term_atom,log)
                         parent.SendMessage(dut_id,'Result: Round %d Pass\n\n'%(cnt+1),log,2)
                      else: 
                         parent.SendMessage(dut_id,'Progress: default Odin check (%s <> %s)\n'%(md5,default_table_md5),log,2) 
                         dload(parent,dut_id,term_arm,term_atom,log)
                         parent.SendMessage(dut_id,'Result: Round %d reload code\n\n'%(cnt+1),log,2)
                      Snmp.SnmpSet("192.168.1.10","1.3.6.1.4.1.26104.1.1.1.5.1.31","i",3,community="public")  #.24 #Power 1 , .25 Power 2, .31 power 8
                      time.sleep(3)
                else:                   
                   if default_table_md5 == md5:
                      parent.SendMessage(dut_id,'Progress: Odin_md5= %s (==%s)\n'%(md5,default_table_md5),log,2)
                      Enter_Testmode(parent,dut_id,term_arm,log)
                      ClitoCal(parent,dut_id,term_arm,log) 
                      if DSCal: mxl277_cmoa_csn(parent,dut_id,term_arm,basePower,bandwidth,cnt,log); parent.SendMessage(dut_id,'Progress: rebooting...\n',log,2)                   
                      Snmp.SnmpSet("192.168.1.10","1.3.6.1.4.1.26104.1.1.1.5.1.31","i",2,community="public")  #.24 #Power 1 , .25 Power 2, .31 power 8
                      time.sleep(5)
                      Snmp.SnmpSet("192.168.1.10","1.3.6.1.4.1.26104.1.1.1.5.1.31","i",1,community="public")  #.24 #Power 1 , .25 Power 2, .31 power 8
                      time.sleep(3)
                   else: flag=0
        failrate=float(failcount)/100; sync_failrate=float(failcount_)/100; timewait_failrate=float(failcount__)/100
        parent.SendMessage(dut_id,'not sync failrate: %.2f\n sync failrate: %.2f\n wait more 3 sec failrate: %.2f'%(failrate,sync_failrate,timewait_failrate),log,2)              
        if failcount or failcount_ or failcount__: result=1  
    except Except,msg:
        parent.SendMessage(dut_id,"\n%s\n"%msg,log,color=1)
        result = 1
    except: 
    ...
    if result:
       ...
       parent.SendMessage(dut_id,"Test Result:PASS"+'\n',log,color=2)
    if log:log.close()
    if term_arm: term_arm.close()
    if term_atom: term_atom.close()
    if mac: 
       global mac_list
       mac_list.remove(mac) 
       if result:
