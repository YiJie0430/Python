...
def mxl214_cal(parent,dut_id,term,basePower,bandwidth,log):
    '''
    author: YiJie Wang
    date: 2016/12/20
    description: *calibrate for MxL214
    '''
    result=0
    test_time = time.time()
    for diplexer in diplexer_list:
      ...
      for t in xrange(3):
        ...
        if 'D/S Calibration Finished' not in data:
          ...
          result=ConfirmM214CalInfo(parent,dut_id,data,DS_FreqComp,DS_FreqComp_offset,log)
...
def ConfirmM214CalInfo(parent,dut_id,data,DS_FreqComp,DS_FreqComp_offset,log): 
    ...
    for i in range(3):
      ...
      message=str()
      for j in range(8):
        if (DS_FreqComp[i][j]-DS_FreqComp_offset) < Segment[i][j] < (DS_FreqComp[i][j]+DS_FreqComp_offset):
          msg='Segment:%d c0= %d (%d ~ %d)'%(j,Segment[i][j],DS_FreqComp[i][j]-DS_FreqComp_offset,DS_FreqComp[i][j]+DS_FreqComp_offset)
        else:
          msg='Segment:%d c0= %d (%d ~ %d) -- fail'%(j,Segment[i][j],DS_FreqComp[i][j]-DS_FreqComp_offset,DS_FreqComp[i][j]+DS_FreqComp_offset)  
          test_fail=1
        message+=msg+"\r"
...
def RF2_insertion_loss(parent,dut_id,term,basePower,bandwidth,log):
    '''
    author: YiJie Wang
    date: 2016/12/19
    description: *RF2_insertion_loss measure
                 *compared with nosie power table
                 *criteria is +/-2 
    '''
    gpib.SetD3StationCal(bandwidth)
    result=0
    test_time=time.time()
    #for diplexer in diplexer_list:
      #freq_range=ds_freqs[diplexer][1]
    attn,ds_data=Read_Get_table(dut_id,freq_insertion,basePower,bandwidth,log)
    pportOut(attn,0x378)
    parent.SendMessage(dut_id,"Start to verify RF2 insertion loss\n",log)
    parent.SendMessage(dut_id,"Frequencies: %s \n"%(ds_data[0]),log)
    parent.SendMessage(dut_id,"Noise Power: %s \n"%ds_data[1],log)
    f_list=ds_data[0].split(' ')
    p_list=ds_data[1].split(' ')
    fp_dict=dict()
    for index,value in enumerate(f_list):
      fp_dict.update([(value,p_list[index])])
    print fp_dict
    for freqs in freq_insertion:
      pwr=gpib.ReadD3StationCalPower(freqs)
      getpwr=float(fp_dict[str(freqs)])
      msg='%.1fMhz noise power=%.2f RF2 measre=%.2f diff=%.2f'%(freqs,getpwr,pwr,(getpwr-pwr))
      if abs(getpwr-pwr)>3:
        msg='%.1fMhz noise power=%.2f RF2 measre=%.2f diff=%.2f ---fail'%(freqs,getpwr,pwr,(getpwr-pwr))        
        parent.SendMessage(dut_id,'%s\n'%msg,log)
        result=1
      else: parent.SendMessage(dut_id,'%s\n'%msg,log)
    parent.SendMessage(dut_id,'RF2 insertion loss Test Done\n',log)
    parent.SendMessage(dut_id,"Insertion loss verification time: %3.2f (sec)\n"%(time.time()- test_time) ,log)
    parent.SendMessage(dut_id,"---------------------------------------------------------------------------\n",log)
    if result: raise Except("FAIL: RF2 insertion loss\n")
...
def T2(parent):
    try:
        ...
        parent.SendMessage(dut_id,"---------------------------------------------------------------------------\n",log)  
        Check_boot_testmode(parent,dut_id,term,log)
        if RF2_loss: RF2_insertion_loss(parent,dut_id,term,basePower,bandwidth,log)
        Enter_Testmode(parent,dut_id,term,log)
        ClitoCal(parent,dut_id,term,log)                        
        if DSCal: mxl214_cal(parent,dut_id,term,basePower,bandwidth,log)
